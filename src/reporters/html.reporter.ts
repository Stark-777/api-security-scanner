...
export class HtmlReporter implements Reporter {
  generate(report: ScanReport): string {
    const summaryItems = severityOrder
      .map((severity) => {
        return `<li><strong>${escapeHtml(severity)}:</strong> ${report.summary.findingsBySeverity[severity] ?? 0}</li>`;
      })
      .join("");
  ...
  <body>
    <main>
      <section class="panel">
        ...
        <p>Unified artifact generated by ${escapeHtml(report.metadata.toolName)}.</p>
        <div class="meta">
          <div><strong>Report version:</strong> ${escapeHtml(report.metadata.reportVersion)}</div>
          <div><strong>Generated at:</strong> ${escapeHtml(report.metadata.generatedAt)}</div>
        </div>
        <div class="summary-grid">
          <div class="summary-card">
            <strong>${report.summary.endpointsScanned}</strong>
